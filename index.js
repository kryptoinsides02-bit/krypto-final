...
const MAX_FEED = 500;  // max items in memory

// How long each type stays "fresh" (used for initial-load filtering)
const ONE_HOUR = 60 * 60 * 1000;
const FRESH_WINDOW = {
  tweet:       ONE_HOUR,        // 1 hour of tweets on login
  news:        ONE_HOUR,        // 1 hour of news
  price_alert: ONE_HOUR,        // 1 hour of price alerts
};

// No LOGIN_CAP — send ALL fresh items within the 1-hour window
...
wss.on('connection', (ws, req) => {
  ...
  const snapshot = feed
    .filter(item => {
      ...
      counts[item.type]++;  // track count for batch_start info only
      return true;
    })
